...
- **Diagnosing a stuck/slow run**: check `curl -s 127.0.0.1:9234/_cluster/health` and look for a large `active_shards` count, and check `log/datastore_client.test.log` for repeated 429 `rejected_execution_exception` errors. Recover by killing the test run and re-booting the datastore (`bundle exec rake elasticsearch:test:boot` or `opensearch:test:boot`), then re-run.
