      ...
      class RelationJoin < ::Data.define(:field, :document_id_field_name, :filter_id_field_name, :id_cardinality, :doc_cardinality, :additional_filter, :foreign_key_nested_paths)
        def self.from(field)
          ...
          if relation.direction == :in
            ...
            # back to the `referenced_field_name` field on the document with the relation.
            #
            ...
            new(field, relation.referenced_field_name, relation.foreign_key, Cardinality::One, doc_cardinality, relation.additional_filter, relation.foreign_key_nested_paths)
          else
            # An outbound foreign key has some field (such as `foo_id`) on the document with the relation
            # that points out to the `referenced_field_name` field of another document.
            new(field, relation.foreign_key, relation.referenced_field_name, doc_cardinality, doc_cardinality, relation.additional_filter, relation.foreign_key_nested_paths)
          end
        end
