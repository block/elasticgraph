  ...
  class Indexer
    module Operation
      class Update < Support::MemoizableData.define(:event, :prepared_record, :destination_index_def, :update_target, :doc_id, :destination_index_mapping)
        ...
        # Returns the bulk request metadata hash used by the datastore. Public so that
        # `CoalescedUpdate` and `DerivedUpdateCoalescer` can access it.
        def metadata
          {
        ...
        # Returns the script parameters for this update. Public so that
        # `CoalescedUpdate` can merge params from multiple source operations.
        def script_params
