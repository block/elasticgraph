...
module ElasticGraph
  class Indexer
    RSpec.describe Processor, :ingests_json_data, :factories, :capture_logs do
      [:json, :proto].each do |format|
        context "with #{format} ingestion" do
          ...
          context "process non-rollover upsert events" do
            describe "upserts" do
              ...
              it "tolerates integer-valued-but-float-typed version values" do
                ...
                event = event.with(version: event.version.to_f)
              ...
              # These events cannot be serialized as protobuf because the primitive types are wrong.
              # Protobuf semantic validation failures are covered by the protobuf ingestion specs.
              context "when an event is malformed", if: format == :json do
          ...
          def process_batches(*batches, via: indexer)
            batches.each do |batch|
              via.processor.process(round_trip_indexing_events(batch), refresh_indices: true)
            end
          end
