    ...
    RSpec.describe SqsProcessor, :capture_logs do
      ...
      let(:json_indexer) do
        JSONIngestion::Indexer.new(instance_double(Indexer, logger: logger)).tap do |json_indexer|
          allow(json_indexer).to receive(:process_decoded_returning_failures).and_return([])
        end
      end
      ...
      describe "#process" do
        ...
        it "processes a lambda event containing a single SQS message with a single ElasticGraph event" do
          ...
          expect(json_indexer).to have_received(:process_decoded_returning_failures).with([
            {"field1" => {}, "message_id" => "a"}
        ...
        it "raises a clear error if the lambda event does not contain SQS messages under `Records` as expected" do
          ...
          expect(json_indexer).not_to have_received(:process_decoded_returning_failures)
        end
        ...
        it "parses and merges SQS timestamps into non-existing `latency_timestamps` field" do
          ...
          expect(json_indexer).to have_received(:process_decoded_returning_failures) do |events|
