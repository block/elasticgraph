  ...
  module JSONIngestion
    ...
    class IngestionAdapter
      ...
      def event_for_json_schema_validation(event, selected_schema_version)
        event.except(INGESTION_FORMAT_KEY, SCHEMA_VERSION_KEY).merge(JSON_SCHEMA_VERSION_KEY => selected_schema_version)
      end
      ...
      def select_schema_version(event)
        ...
        if !event.key?(SCHEMA_VERSION_KEY)
          yield ValidationResult.invalid(validation_target: SCHEMA_VERSION_KEY, message: "Event lacks a `#{SCHEMA_VERSION_KEY}`")
        elsif !requested_schema_version.is_a?(Integer) || requested_schema_version < 1
          yield ValidationResult.invalid(validation_target: SCHEMA_VERSION_KEY, message: "#{SCHEMA_VERSION_KEY} (#{requested_schema_version}) must be a positive integer.")
        end
        ...
        if selected_schema_version.nil?
          yield ValidationResult.invalid(
            validation_target: SCHEMA_VERSION_KEY,
      ...
      def raise(*args)
        super("`raise` was called on `JSONIngestion::IngestionAdapter`, but should not. Instead, return or yield " \
