...
module ElasticGraph
  module JSONIngestion
    ...
    # against the JSON schema identified by the event's `schema_version`, and prepares
    ...
    class IngestionAdapter
      ...
      # presence of the `schema_version` field in the event envelope. The JSON Lines decoder maps
      # the publisher's `json_schema_version` onto that ingestion-format-neutral key.
      #
      ...
      def validate_event(event)
        ...
        if (error_message = validator(graphql_type_name, selected_schema_version).validate_with_error_message(record))
      ...
      # closest available version by design.
      def event_for_json_schema_validation(event, selected_schema_version)
        event.except(SCHEMA_VERSION_KEY).merge(JSON_SCHEMA_VERSION_KEY => selected_schema_version)
      end

      def select_schema_version(event)
        available_schema_versions = @schema_artifacts.available_json_schema_versions

        requested_schema_version = event[SCHEMA_VERSION_KEY]

        # First check that a valid value has been requested (a positive integer)
        if !event.key?(SCHEMA_VERSION_KEY)
          yield ValidationResult.invalid(payload_description: SCHEMA_VERSION_KEY, message: "Event lacks a `#{SCHEMA_VERSION_KEY}`")
        elsif !requested_schema_version.is_a?(Integer) || requested_schema_version < 1
          yield ValidationResult.invalid(payload_description: SCHEMA_VERSION_KEY, message: "#{SCHEMA_VERSION_KEY} (#{requested_schema_version}) must be a positive integer.")
        end
        ...
        if selected_schema_version.nil?
          yield ValidationResult.invalid(
            ...
            message: "Failed to select schema version. Requested version: #{event[SCHEMA_VERSION_KEY]}. \
