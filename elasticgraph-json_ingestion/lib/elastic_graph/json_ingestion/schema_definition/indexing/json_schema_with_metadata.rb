  ...
  module JSONIngestion
    module SchemaDefinition
      module Indexing
        ...
        # The result includes the merged JSON schema plus details about schema elements
        # from prior versions that could not be matched to current metadata.
        #
        # @!attribute [r] json_schema
        #   @return [Hash<String, Object>] the merged JSON schema
        # @!attribute [r] missing_fields
        #   @return [Set<String>] fields (in the form `Type.field`) that were needed but not found
        # @!attribute [r] missing_types
        #   @return [Set<String>] type names that were needed but not found
        # @!attribute [r] definition_conflicts
        #   @return [Set<DeprecatedElement>] deprecated elements that create conflicting definitions
        # @!attribute [r] missing_necessary_fields
        #   @return [Array<MissingNecessaryField>] deleted fields that must be retained for routing or rollover
        #
        # @private
            ...
            def identify_missing_necessary_fields(json_schema, old_type_name_by_current_name)
              ...
              @state.object_types_by_name.values
                ...
                .flat_map do |object_type|
                  ...
                  identify_missing_necessary_fields_for_index_def(index_def, json_schema_resolver)
                end
            end

            def identify_missing_necessary_fields_for_index_def(index_def, json_schema_resolver)
              {
                ...
                if json_schema_resolver.necessary_path_missing?(field_path)
                  # The JSON schema artifact has no field that maps to the #{field_type} path of `#{field_path.fully_qualified_path_in_index}`.
