    ...
    module SchemaDefinition
      ...
      module ResultsExtension
        ...
        def json_schema_indexing_field_types_by_name
          ...
          # runs the `on_built_in_types` callbacks (registered by `APIExtension.extended`) that attach
          # `json_schema_options` onto the built-in scalar types — those options must be set before we
          # call `to_indexing_field_type` on any of them or the resulting JSON schema is incomplete.
          all_types
