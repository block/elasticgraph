  ...
  module ProtoIngestion
    ...
    class Indexer
      ...
      def initialize(indexer, config:)
        ...
        @ingestion_adapter = indexer.ingestion_adapters_by_format.fetch("proto") # : IngestionAdapter
      end
      ...
      def process(payload, metadata: {}, refresh_indices: false)
        decoded_events = decode(payload, metadata: metadata)
      ...
      #
      # @param decoded_events [Array<Hash<String, Object>>] decoded protobuf events, as returned by {#decode}
      # @param refresh_indices [Boolean] whether to synchronously refresh affected indices (intended for tests: this is dangerous to use in production)
