  ...
  module ProtoIngestion
    ...
    class IngestionAdapter
      ...
      def validate_event(event, skip_record_validation: false)
        ...
        ::ElasticGraph::Indexer::IngestionAdapter::ValidationResult.valid(@schema.record_preparer,
          event: event.merge("record" => record, "type" => type))
      end

      private

      def invalid(description, error)
        ::ElasticGraph::Indexer::IngestionAdapter::ValidationResult.invalid(validation_target: description, message: error)
      end
