...
module ElasticGraph
  module ProtoIngestion
    RSpec.describe "Protobuf ingestion", :uses_datastore, :builds_indexer, :builds_graphql, :builds_admin, :capture_logs do
      include CompiledProtoSupport

      it "indexes sourced_from updates from a type without an index" do
        results = define_proto_schema_results do |s|
          s.object_type "CatalogEntry" do |t|
            t.field "id", "ID!"
          ...
          s.object_type "Product" do |t|
            ...
            t.relates_to_one "catalog_entry", "CatalogEntry", via: "product_id", dir: :in, indexing_only: true
            t.field("catalog_name", "String") { |f| f.sourced_from "catalog_entry", "name" }
        ...
        with_compiled_proto(results) do |pool, path|
          ...
          catalog_event = envelope.new(op: "upsert", id: "s1", version: 1, record_catalog_entry: {product_id: "p1", name: "catalog_entry"})
          product = envelope.new(op: "upsert", id: "p1", version: 1, record_product: {label: "product"})
          proto_indexer = Indexer.new(indexer, config: {"descriptor_set_file" => path})
          [catalog_event, product, catalog_event].each do |event|
            proto_indexer.process(batch.encode(batch.new(events: [event])), refresh_indices: true)
          end
          graphql = build_graphql(datastore_core: indexer.datastore_core)
          expect(graphql.graphql_query_executor.execute("{ products { nodes { id label catalog_name } } }").dig("data", "products", "nodes"))
            .to eq([{"id" => "p1", "label" => "product", "catalog_name" => "catalog_entry"}])
        end
      end
        ...
        context "with #{syntax}" do
          ...
          it "indexes real protobuf batches and queries the indexed values without JSON schema artifacts" do
            ...
            with_compiled_proto(results) do |pool, descriptor_file|
              ...
              proto_indexer = Indexer.new(indexer, config: {"descriptor_set_file" => descriptor_file})
              ...
              proto_indexer.process(payload, refresh_indices: true)
              ...
              proto_indexer.process(old_payload, refresh_indices: true)
          ...
          it "writes protobuf records to an unversioned warehouse partition through the shared processor" do
            with_compiled_proto(results) do |pool, descriptor_file|
              ...
              proto_indexer = Indexer.new(warehouse_indexer, config: {"descriptor_set_file" => descriptor_file, "format" => "raw"})
              proto_indexer.process(product.encode(product.new(name: "warehouse", untyped: '{"n":3.0}')),
          ...
          it "accepts raw records with transport properties and isolates invalid envelopes" do
            with_compiled_proto(results) do |pool, descriptor_file|
              product = pool.lookup("events.v1.Product").msgclass
              proto_indexer = Indexer.new(indexer, config: {"descriptor_set_file" => descriptor_file, "format" => "raw", "encoding" => "base64"})
              ...
              events = proto_indexer.decode(payload, metadata: properties)
              expect(events.first).not_to include("schema_version", "json_schema_version")
              failures = proto_indexer.process_returning_failures(
                events + proto_indexer.decode(payload, metadata: properties.merge("eg_version" => "bad")),
