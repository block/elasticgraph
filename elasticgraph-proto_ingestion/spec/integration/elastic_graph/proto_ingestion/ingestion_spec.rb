        ...
        context "with #{syntax}" do
          ...
          it "indexes real protobuf batches and queries the indexed values without JSON schema artifacts" do
            ...
            with_compiled_proto(results) do |pool, descriptor_file|
              ...
              decoder = IndexingEventDecoder.new(config: {"descriptor_set_file" => descriptor_file}, schema_artifacts: results)
          ...
          it "passes SQS attributes to the raw decoder and returns only failed message IDs" do
            with_compiled_proto(results) do |pool, descriptor_file|
              product = pool.lookup("events.v1.Product").msgclass
              configured_indexer = Indexer.new(indexer, config: {
                "descriptor_set_file" => descriptor_file, "format" => "raw", "encoding" => "base64"
              })
              sqs_processor = IndexerLambda::SqsProcessor.new(configured_indexer,
                ignore_sqs_latency_timestamps_from_arns: Set.new)
          ...
          it "writes protobuf records to an unversioned warehouse partition through the shared processor" do
            with_compiled_proto(results) do |pool, descriptor_file|
              ...
              decoder = IndexingEventDecoder.new(config: {"descriptor_set_file" => descriptor_file, "format" => "raw"}, schema_artifacts: results)
          ...
          it "accepts raw records with transport properties and isolates invalid envelopes" do
            with_compiled_proto(results) do |pool, descriptor_file|
              product = pool.lookup("events.v1.Product").msgclass
              decoder = IndexingEventDecoder.new(config: {"descriptor_set_file" => descriptor_file, "format" => "raw", "encoding" => "base64"}, schema_artifacts: results)
