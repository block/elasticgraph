...
--- a/Rakefile
+++ b/Rakefile
@@ -2,5 +2,6 @@ project_root = File.expand_path(__dir__)
...
@@ -15,4 +16,5 @@ ElasticGraph::Local::RakeTasks.new(
 ) do |tasks|
...
For custom scalar types, when `elasticgraph-json_ingestion` is also in use, the generator infers proto
scalar types from `json_schema type:` when it is one of `string`, `boolean`, `number`, or `integer`.
You can override inference (or supply the type when no JSON schema is defined) with `proto_field`:
