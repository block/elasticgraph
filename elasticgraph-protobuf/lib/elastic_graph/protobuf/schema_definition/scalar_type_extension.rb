    ...
    module SchemaDefinition
      ...
      module ScalarTypeExtension
        ...
        def infer_proto_field_type_from_json_schema
          ...
          # `json_schema_options` is provided by `elasticgraph-json_ingestion`'s scalar type extension
          # when that gem's schema definition extension is also in use; Steep can't see that optional
          # runtime coupling, so we have to opt out of type checking the call.
          raw_type = (_ = self).json_schema_options[:type]
          ...
          when String, Symbol
            types = [raw_type.to_s]
          when Array
            types = raw_type.filter_map do |entry|
          ...
          PROTO_FIELD_TYPE_BY_JSON_SCHEMA_TYPE[normalized_types.fetch(0)]
        end
      end
    end
  end
