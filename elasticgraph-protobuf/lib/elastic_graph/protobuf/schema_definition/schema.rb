      ...
      class Schema
        ...
        def renamed_public_field_names_by_type_name
          @renamed_public_field_names_by_type_name ||= begin
            ...
            # `renamed_from` field metadata can only be declared via an extension that adds
            # `renamed_fields_by_type_name_and_old_field_name` to the schema definition state (such as
            # `elasticgraph-json_ingestion`, which is optional). Without such an extension, no rename
            # metadata can exist, so there are no field-number mappings to migrate.
            if @state.respond_to?(:renamed_fields_by_type_name_and_old_field_name)
              renaming_state = @state # : untyped

              renaming_state.renamed_fields_by_type_name_and_old_field_name.each do |type_name, old_to_new|
