    ...
    module SchemaDefinition
      RSpec.describe Schema, :proto_schema do
        ...
        it "generates a single message for an indexed abstract type, covering its subtypes' fields" do
          ...
          expect(generated).to include("message Vehicle {")
          expect(generated).to include("string id =", "int32 doors =", "int32 gears =")
          expect(generated).not_to include("message Car {", "message Bike {")
        end
        ...
        it "uses explicit proto_field mappings independent of json_schema type" do
          results = define_proto_schema do |s|
        ...
        it "generates the proto schema and assigns field numbers when used without `elasticgraph-json_ingestion`" do
          # Overrides the extension modules to activate ONLY the protobuf extension. Without
          # `elasticgraph-json_ingestion`, `renamed_from` cannot be declared, so no rename
          # metadata exists on the schema definition state for proto generation to consult.
          results = define_proto_schema(extension_modules: [SchemaDefinition::APIExtension]) do |s|
            s.object_type "Account" do |t|
              t.field "id", "ID"
