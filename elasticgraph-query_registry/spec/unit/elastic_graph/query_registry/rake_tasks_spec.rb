        ...
        it "reports issues with variables" do
          ...
          expect(last_task_output.string.strip).to eq(<<~EOS.strip)
            For client `client_bob`:
              - CountWidgets.graphql (1 operation):
                - CountWidgets: 🛑. Got 1 validation error:
                  1) No dumped variables for this operation exist. Correct by running: `bundle exec rake "query_registry:dump_variables[client_bob, CountWidgets]"`


            For client `client_jane`:
              - CountWidgets.graphql (1 operation):
                - CountWidgets: 🛑. Got 1 validation error:
                  1) No dumped variables for this operation exist. Correct by running: `bundle exec rake "query_registry:dump_variables[client_jane, CountWidgets]"`
      ...
      describe "query_validator:dump_variables" do
        ...
        def expect_dumped_variables_files(*paths)
          ...
          expected_files.each do |file|
            ...
            expect(contents).to include("Generated by `bundle exec rake \"query_registry:dump_variables[#{client_name}, #{query_name}]\"`.")
