  ...
  module SchemaDefinition
    module Mixins
      ...
      module HasTypeInfo
        ...
        # Returns the JSON schema options recorded for this element.
        #
        # The default implementation returns an empty hash; ingestion-serializer extensions (such as
        # `elasticgraph-json_ingestion`) override this on the elements they extend to return real options.
        # Defining it here lets call sites (and Apollo/test helpers) reference `json_schema_options` and
        # call `json_schema(...)` without having to detect whether an ingestion serializer is loaded.
        #
        # @return [Hash<Symbol, Object>] JSON schema options
        def json_schema_options
          # :nocov: -- only reached when no ingestion-serializer extension is loaded; the JSON Ingestion
          # extension shadows this method on every element it extends.
          {}
          # :nocov:
        end

        # Records JSON schema options for this element.
        #
        # The default implementation is a no-op; an ingestion-serializer extension overrides this on the
        # elements it extends to apply real validation and storage. The default JSON ingestion extension
        # provides the user-facing behavior and documentation for this API.
        #
        # @param options [Hash<Symbol, Object>] JSON schema options
        # @return [void]
        def json_schema(**options)
          # :nocov: -- only reached when no ingestion-serializer extension is loaded.
          # :nocov:
        end
