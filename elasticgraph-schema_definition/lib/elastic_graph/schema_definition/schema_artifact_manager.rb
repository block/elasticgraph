  ...
  module SchemaDefinition
    ...
    class SchemaArtifactManager
      ...
      def new_yaml_artifact(file_name, desired_contents, extra_comment_lines: [])
        SchemaArtifact.new(
          file_name: ::File.join(@schema_artifacts_directory, file_name),
          desired_contents: desired_contents,
          dumper: ->(hash) { ::YAML.dump(hash) },
          loader: ->(string) { ::YAML.safe_load(string) },
      ...
      def new_raw_artifact(file_name, desired_contents, comment_prefix: "#")
        SchemaArtifact.new(
          file_name: ::File.join(@schema_artifacts_directory, file_name),
          desired_contents: desired_contents,
          dumper: _ = :itself.to_proc,
          loader: _ = :itself.to_proc,
          extra_comment_lines: [],
    ...
    # @private
    class SchemaArtifact < Support::MemoizableData.define(
      :file_name,
      :desired_contents,
      :dumper,
      :loader,
      :extra_comment_lines,
      :comment_prefix,
      :comment_preamble_lines
    )
      DEFAULT_COMMENT_PREAMBLE_LINES = [
      ...
      def comment_preamble
        comment_preamble_lines_with_extras.map { |line| "#{comment_prefix} #{line}".rstrip }.join("\n")
      end

      def comment_preamble_lines_with_extras
        return comment_preamble_lines if extra_comment_lines.empty?
