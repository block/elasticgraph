  ...
  class WarehouseLambda
    ...
    class WarehouseDumper
      ...
      def bulk(operations, refresh: false)
        operations_by_type_and_json_schema_version = operations.group_by { |op| [op.event.type, op.event.schema_version] } # : ::Hash[[::String, ::Integer?], ::Array[::ElasticGraph::Indexer::operation]]
      ...
      def build_jsonl_file_from(operations)
        operation_payloads = operations.filter_map do |op|
          ...
          next nil if op.update_target.type != op.event.type
