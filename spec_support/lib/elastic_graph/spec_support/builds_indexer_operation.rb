...
module ElasticGraph
  module SpecSupport
    ...
    module BuildsIndexerOperation
      ...
      # @return [Indexer::Operation::Update] The primary indexing operation
      def new_primary_indexing_operation(event, index_def: nil, idxr: indexer)
        ...
        Indexer::Operation::Update.new(
          event: event,
          prepared_record: idxr.record_preparer_factory.for_latest_schema_version.prepare_for_index(
            event.fetch("type"),
            event.fetch("record"),
