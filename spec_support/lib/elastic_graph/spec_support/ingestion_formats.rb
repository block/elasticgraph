...
RSpec.shared_context "ingestion format support" do |format|
  ...
  let(:ingestion_schema_artifacts) do
    if format == :json
      ...
      define_schema(schema_element_name_form: :snake_case, extension_modules: [ElasticGraph::ProtoIngestion::SchemaDefinition::APIExtension]) do |schema|
    ...
    if format == :json
      ...
      with_compiled_proto(ingestion_schema_artifacts) do |pool, path|
        ...
        @ingestion_proto_decoder = ElasticGraph::ProtoIngestion::IndexingEventDecoder.new(
          config: {"descriptor_set_file" => path}, schema_artifacts: ingestion_schema_artifacts
        )
        @ingestion_proto_adapter = ElasticGraph::ProtoIngestion::IngestionAdapter.new(schema_artifacts: ingestion_schema_artifacts)
  ...
  def round_trip_indexing_events(events)
    ...
    envelopes = events.map do |event|
      ...
      attributes = {op: event.op, id: event.id, version: event.version, latency_timestamps: event.latency_timestamps}
      attributes[field] = proto_json_value(type, event.record)
      envelope_class.decode_json(JSON.generate(attributes))
    end
    decoded_events = @ingestion_proto_decoder.decode(batch_class.encode(batch_class.new(events: envelopes)))
    @ingestion_proto_adapter.events_from(decoded_events).first
  end
