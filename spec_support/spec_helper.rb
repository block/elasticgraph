...
#
# simplecov:disable -- only one side of this conditional gets executed on any given test run.
if ENV["SUPERDIFF"]
...
RSpec.configure do |config|
  ...
  config.around(:example) do |ex|
    ...
    # simplecov:disable -- we only get here when we forget to use `abort_with`, so this is not covered on a normal, passing test suite run.
    fail <<~EOS
  ...
  # simplecov:disable -- only one side of this conditional is covered on any given Ruby engine.
...
module ElasticGraph
  module CommonSpecHelpers
    ...
    def self.datastore_backend=(backend)
      # simplecov:disable -- the `raise` is only hit when the method is called more than once, which should never happen.
    ...
    # simplecov:disable -- any given run either runs with VCR loaded or w/o it loaded, and won't cover all branches here.
    def without_vcr
    ...
    # `nil` selects the minimal format-neutral ingestion extension for inline test schemas.
    # Pass an explicit array when a spec needs to control the registered extensions.
    def generate_schema_artifacts(
      ...
      if block_given?
        extension_modules ||= [SchemaDefinition::TestSupport::APIExtension]
      else
        ...
        extension_modules = Array(extension_modules) + [JSONIngestion::SchemaDefinition::APIExtension]
      end
